...
import com.mectris.mectrispaper.utils.GeoIpResolver;
...
public class PlayerSessionCollector implements Listener {
    ...
    public void onQuit(@NotNull PlayerQuitEvent event) {
        ...
        if (active != null) enqueueSession(event.getPlayer().getUniqueId(), active);
    }
    ...
    private void enqueueSession(@NotNull UUID uuid, @NotNull Active active) {
    ...
    private void trackCountry(UUID uuid, InetSocketAddress socket) {
        var address = socket != null ? socket.getAddress() : null;

        GeoIpResolver.resolve(address, code -> {
